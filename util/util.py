...
def save_network(net, label, epoch, opt):
    ...
    save_path = os.path.join(opt.output_path, "saved_models", save_filename)
...
class Colorize(object):
    def __init__(self, n=35):
        ...
        self.cmap = jittor.uint8(self.cmap[:n])

    def __call__(self, gray_image):
        size = gray_image.size()
        color_image = jittor.zeros((3, size[1], size[2]), dtype="uint8")
